    ...
    with torch.no_grad():
        ...
        for i, row in df.iterrows():
            ...
            if not use_pred:
                ...
                qidxs, ctrues, cpreds = predict_each_group2(dtotal, dcur, curdforget, is_repeat, qidx, uid, idx, model_name, model, t, end, fout, atkt_pad)
                # 计算
                ...
                while t < curl:
                    ...
                    curqin, curcin, currin, curtin, curdforget, ctrues, cpreds = predict_each_group(dtotal, dcur, curdforget, is_repeat, qidx, uid, idx, model_name, model, t, end, fout, atkt_pad)
...
def predict_each_group(dtotal, dcur,  curdforget, is_repeat, qidx, uid, idx, model_name, model, t, end, fout, atkt_pad=False, maxlen=200):
...
def prepare_data(model_name, is_repeat, qidx, dcur, curdforget, dtotal, t, end, maxlen=200):
    curqin, curcin, currin, curtin = dcur["curqin"], dcur["curcin"], dcur["currin"], dcur["curtin"]
    cq, cc, cr, ct = dtotal["cq"], dtotal["cc"], dtotal["cr"], dtotal["ct"]
...
def predict_each_group2(dtotal, dcur, curdforget, is_repeat, qidx, uid, idx, model_name, model, t, end, fout, atkt_pad=False, maxlen=200):
    ...
    # 以下这些用的是同一个历史,可以并行
    # 不用预测结果
    qidxs, finalqs, finalcs, finalrs, finalts, finalqshfts, finalcshfts, finalrshfts, finaltshfts, finald, finaldshft = prepare_data(model_name, is_repeat, qidx, dcur, curdforget, dtotal,  t, end)
    bidx, bz = 0, 128
